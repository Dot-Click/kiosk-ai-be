...
const verifySession = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (order) {
            ...
            return SuccessHandler_1.SuccessHandler.handle(res, "Order verified (already existed)", order, 200);
        }
        const stripe = new stripe_1.default(config.secretKey);
        ...
        order = yield Order_1.default.findOne({ "payment.stripeSessionId": sessionId });
        if (order) {
        ...
        return SuccessHandler_1.SuccessHandler.handle(res, "Order verified and created", order, 201);
    }
    catch (error) {
        ...
        if (error.code === 11000) {
            ...
            const existingOrder = yield Order_1.default.findOne({ "payment.stripeSessionId": sessionId });
            if (existingOrder) {
                return SuccessHandler_1.SuccessHandler.handle(res, "Order verified (recovered from race)", existingOrder, 200);
            }
        }
        return ErrorHandler_1.ErrorHandler.handleError(new ErrorHandler_1.ApiError(500, error.message || "Failed to verify order"), req, res);
    }
});
